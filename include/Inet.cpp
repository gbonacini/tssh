...
namespace inet {
   ...
   ssize_t Inet::readBuffer(size_t len, Handler* hdlr, void** buff) noexcept(false){
      ...
      if(readLen < 1 && errno != EINTR){
          readLen == 0 ? throw InetException("Connection was closed by the server."):
               throw InetException(string("readBuffer: Read error: ") + strerror(errno));
      }
      errno   = 0;
   ...
   ssize_t Inet::readBufferNb(size_t len, Handler* hdlr, void** buff) noexcept(false){
      ...
      if(tlen < 1 && errno != EAGAIN && errno != EINTR){
          tlen == 0 ? throw InetException("Connection was closed by the server."):
